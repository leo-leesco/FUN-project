...
let lookup : atom -> tenv -> ftype =
 fun x tenv ->
  ...
  with Not_found ->
    assert false (* should not happen, if there is a binding of [x] in
[tenv] *)
...
let jlookup : atom -> jenv -> atom list * ftype list =
 fun x jenv ->
  ...
  with Not_found ->
    assert
      false (* should not happen, if
    there is a binding of [x] in [tenv] *)
